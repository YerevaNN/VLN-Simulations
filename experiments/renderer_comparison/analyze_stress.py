...
keys+=['omni51-hq-ap','omni51-pt-ap','omni51-hq','omni51-pt']
labels+=['A6000 / OmniGibson settings (legacy RT)','A6000 / latest OmniGibson 5.1 PT','H100 / OmniGibson settings (legacy RT)','H100 / latest OmniGibson 5.1 PT']
keys+=['omni51-kit-hq-ap','omni51-kit-hq']
labels+=['A6000 / OmniGibson Kit 5.1 HQ','H100 / OmniGibson Kit 5.1 HQ']
available=[(k,l) for k,l in zip(keys,labels) if (root/k/'manifest.json').exists()]
available.sort(key=lambda x: {'omni51-kit-hq-ap':0,'omni51-kit-hq':1}.get(x[0],2))
...
def copy_run(k):
    ...
    for f in (root/k).iterdir():
        if f.is_file():shutil.copy2(f,out/k/f.name)
metrics=[]
for n in names:
    for k in keys:
...
if (root/ref/'manifest.json').exists():
    ...
    copy_run(ref)
hardware=[]
for h,a in [('omni51-hq','omni51-hq-ap'),('omni51-kit-hq','omni51-kit-hq-ap')]:
    if h not in keys or a not in keys:continue
    for n in names:
        hd=np.load(root/h/(n+'-depth.npy')).squeeze(); ad=np.load(root/a/(n+'-depth.npy')).squeeze()
        mask=np.isfinite(hd)&np.isfinite(ad)&(hd>0)&(ad>0)&(hd<100)&(ad<100)
        hardware.append(dict(left=h,right=a,view=n,mae=float(np.abs(im(h,n)-im(a,n)).mean()),depth_valid_fraction=float(mask.mean()),depth_abs_p95_m=float(np.percentile(np.abs(hd[mask]-ad[mask]),95))))
(out/'metrics.json').write_text(json.dumps(dict(pairs=metrics,repeats=repeats,diagnostic=diagnostic,hardware=hardware),indent=2))
...
if 'omni51-kit-hq' in keys and 'omni51-kit-hq-ap' in keys:
    pair=Image.new('RGB',(1280,400),'#172028'); pd=ImageDraw.Draw(pair)
    for col,(k,label) in enumerate([('omni51-kit-hq-ap','A6000 / OmniGibson + Isaac 5.1 RT 2.0'),('omni51-kit-hq','H100 / OmniGibson + Isaac 5.1 RT 2.0')]):
        pd.text((col*640+8,8),label,fill='white',font=font)
        pair.paste(Image.open(root/k/'transparency_2.png'),(col*640,40))
    pair.save(out/'omni51-pair.png')
...
if not any(k in keys for k in ['behavior45-pt','omni51-hq','omni51-pt','omni51-kit-hq']):
...
page=page.replace('<option>transparency_2</option>','<option selected>transparency_2</option>')
