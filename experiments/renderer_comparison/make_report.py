...
labels={'isaac-ap':'Isaac Sim 5.1 · AP / RTX A6000',
        ...
        'behavior45-pt':'BEHAVIOR / Isaac 4.5 · H100',
...
<p><a href="https://github.com/YerevaNN/VLN-Simulations/blob/codex/h100-renderer-comparison/experiments/renderer_comparison/RESULTS.md">Experiment findings</a> · <a href="comparison.png">Comparison image</a></p>
...
for(let i=0;i<3;i++){const card=document.createElement('article');const options=Object.entries(data.backends).map(([k,v])=>`<option value="${k}">${v.label}</option>`).join('');card.innerHTML=`<select>${options}<option value="unavailable">Isaac 5.1 on H100 · unsuccessful</option>${data.backends.newton?'':'<option value="newton-unavailable">No completed Newton render</option>'}</select><div class="picture"></div><div class="meta"></div>`;card.querySelector('select').value=data.default[i];card.querySelector('select').onchange=render;document.querySelector('#grid').append(card)}
